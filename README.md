...
├── logs/                   # 日志目录
│   └── crypto_alert.log    # 日志文件
├── main.py                 # 主程序入口
├── run_backtest.py         # 回测服务入口
└── generate_report.py      # 性能报告生成工具
```
...
# 价格与EMA21的偏离比例阈值，小于此值会得到1分
price_ema_gap_ratio = 0.003

# ATR放大倍数阈值，大于等于此值会得到2分
atr_ratio = 1.1

# 成交量放大倍数阈值，大于等于此值会得到2分
volume_ratio = 1.3
...
4. 配置飞书机器人webhook地址和其他参数：
   - 修改 `config/alert_config.ini` 中的 `feishu_webhook` 为你的飞书机器人webhook地址
   - 根据需要调整其他配置参数
...
- Linux/Mac (crontab)：
  ```
  */5 * * * * cd /path/to/crypto_alert && python main.py
  ```
- Windows (任务计划程序)：
  创建基本任务，设置触发器为每5分钟运行一次，操作为启动程序 `python`，参数为 `main.py`，起始位置为项目目录

### 回测功能

有两种方式运行回测：

1. 单次回测：`python main.py --backtest`
2. 持续回测服务：`python run_backtest.py`（每小时自动执行回测任务）

回测功能会自动检查已发送的告警，并记录1小时、4小时和24小时后的价格变化情况。

### 性能分析

生成性能分析报告：`python generate_report.py [--days 天数] [--output 输出文件路径]`
...
项目包含多个测试脚本，用于测试系统的各个组件和功能。详细说明请参考 `tests/README.md`。
...
系统使用多项技术指标为交易信号评分，满分为10分，默认阈值为6分：
...
3. **价格与EMA贴近度**（最高1分）：
   - 价格与EMA偏离比例小于阈值（默认0.3%）：+1分
   - 示例：价格偏离EMA仅0.2%，小于0.3%阈值，得1分

4. **ATR放大**（最高2分）：
   - ATR放大倍数大于等于阈值（默认1.1倍）：+2分
   - 示例：ATR放大1.5倍，大于1.1倍阈值，得2分

5. **成交量放大**（最高2分）：
   - 成交量放大倍数大于等于阈值（默认1.3倍）：+2分
   - 示例：成交量放大2倍，大于1.3倍阈值，得2分
...
2. **信号分数变化显著**：信号分数相比上次提醒变化超过阈值（默认2分）
   - 示例：SOL上次提醒分数6分，当前分数9分（变化3分），超过2分阈值，满足条件
   - 示例：AVAX上次提醒分数7分，当前分数8分（变化1分），低于2分阈值，不满足条件

3. **RSI变化显著**（如启用）：RSI相比上次提醒变化超过阈值（默认5点）
   - 示例：XRP上次提醒RSI为45，当前RSI为55（变化10点），超过5点阈值，满足条件
   - 示例：BNB上次提醒RSI为50，当前RSI为53（变化3点），低于5点阈值，不满足条件
